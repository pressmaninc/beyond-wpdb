...
class Beyond_Wpdb_Meta_Query_Test extends Beyond_Wpdb_Test {
	...
	public function test_get_meta_table_success()
	{
		...
		foreach ( $metaKey as $type ) {
			$expected_table_name = $wpdb->prefix . $type. 'meta_beyond';
	...
	public function test_get_sql_join()
	{
		...
		$expected_join = 'INNER JOIN wptests_postmeta_beyond AS wptests_postmeta_beyond ON ( wptests_posts.id = wptests_postmeta_beyond.post_id )';
		$type = 'post';
		$queries = array(
			array(
				'key'     => 'region',
	...
	public function test_get_sql_where_equal_or_exists()
	{
		...
		$expected_where = $this->remove_spaces(
			"AND ( JSON_UNQUOTE( JSON_EXTRACT(wptests_postmeta_beyond.json, '$.region') ) = 'tokyo' 
					AND JSON_UNQUOTE( JSON_EXTRACT(wptests_postmeta_beyond.json, '$.language') ) = 'japanese' )"
		);
	...
	public function test_get_sql_where_like_or_not_like()
	{
		...
		$expected_where = $this->remove_spaces(
			"AND ( JSON_UNQUOTE( JSON_EXTRACT(wptests_postmeta_beyond.json, '$.region') ) LIKE $metaValue1 
					AND JSON_UNQUOTE( JSON_EXTRACT(wptests_postmeta_beyond.json, '$.language') ) NOT LIKE $metaValue2 )"
		);
	...
	public function test_get_sql_where_in_or_not_in()
	{
		...
		$json = 'wptests_postmeta_beyond.json';
